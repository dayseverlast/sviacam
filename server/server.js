...
server.on("connection", function(websocket) {
    ...
    websocket.on('message', function(data) {
		...
		try {
			...
			x = array[0]>0?array[0]:-1; 
			y = array[1]>0?array[1]:-1; 
			w = array[2]>0?array[2]:-1; 
			h = array[3]>0?array[3]:-1;
		} catch (e){
			console.error("Erreur parse coordonnees de selection:", e)
		}
		// affiche les coordonnees de la selection si c'est est une (pas un simple click sans drag dans canvas
